...
describe("YoutubeService", () => {
  ...
  const mockOAuthToken =
    "ya29.AHES6ZQO3-vRzMlkNE1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k1k";
  ...
  describe("fetchSingleYoutubeVideo", () => {
    describe("OAuth token authentication", () => {
      it("should pass OAuth token correctly to content service", async () => {
        // Arrange
        mockContentService.fetchSingleYoutubeVideo.mockResolvedValue(
          mockVideoResult,
        );
        ...
        expect(
          mockContentService.fetchSingleYoutubeVideo,
      ...
      it("should handle OAuth token with typical Bearer format", async () => {
        ...
        // Act
        await youtubeService.fetchSingleYoutubeVideo(
          bearerToken,
          "OAUTH",
          mockVideoUrl,
        );
    ...
    describe("Parameter passing validation", () => {
      it("should pass both authToken and videoUrl parameters in correct order", async () => {
        ...
        const callArgs =
          mockContentService.fetchSingleYoutubeVideo.mock.calls[0];
    ...
    describe("Error handling from content service", () => {
      it("should propagate errors from content service without modification", async () => {
        ...
        mockContentService.fetchSingleYoutubeVideo.mockRejectedValue(
          contentServiceError,
        );

        // Act & Assert
        await expect(
          youtubeService.fetchSingleYoutubeVideo(
            mockOAuthToken,
            "OAUTH",
            mockVideoUrl,
          ),
  ...
  describe("Content service integration tests", () => {
    describe("Authentication method detection via service calls", () => {
      it("should correctly handle OAuth tokens with Authorization headers", async () => {
        // Arrange
        const oauthToken =
          "ya29.Long_OAuth_Token_With_Dots.And-Dashes_123456789012345678901234567890123456789012345678901234567890";

        // Mock the content service to capture how it's called
        let capturedRequestConfig: any = null;
        mockContentService.fetchSingleYoutubeVideo = vi
          .fn()
          .mockImplementation(async (token, url) => {
    ...
    describe("Explicit Authentication Method Usage", () => {
      it("should correctly pass OAuth authentication method to content service", async () => {
        // Arrange
        const oauthToken =
          "ya29.Long_OAuth_Token_Format_With_Special.Characters-123456789";

        mockContentService.fetchSingleYoutubeVideo = vi
          .fn()
          .mockResolvedValue(mockVideoResult);
      ...
      it("should maintain compatibility with buildRequestConfig for different token types", async () => {
        ...
        const testCases = [
          {
            token:
              "ya29.OAuth_Token_With_Dots.And-Dashes-Very_Long_Format_123456789",
        ...
        for (const testCase of testCases) {
          ...
          // Act
          const authMethod =
            testCase.expectedAuthType === "oauth" ? "OAUTH" : "API_KEY";
          await youtubeService.fetchSingleYoutubeVideo(
            testCase.token,
            authMethod,
            mockVideoUrl,
          ...
          expect(
            mockContentService.fetchSingleYoutubeVideo,
          ).toHaveBeenCalledWith(testCase.token, authMethod, mockVideoUrl);
        }
      });
    });
  });
