...
describe("YoutubeContentService - Explicit Authentication Method", () => {
  ...
  describe("fetchSingleYoutubeVideo", () => {
    ...
    it("should handle OAuth authentication correctly", async () => {
      ...
      (service as any).httpClient = mockAxios;

      await service.fetchSingleYoutubeVideo(oauthToken, "OAUTH", videoUrl);

      // Verify that OAuth authorization header was used
    ...
    it("should handle API key authentication correctly", async () => {
      ...
      await service.fetchSingleYoutubeVideo(apiKey, "API_KEY", videoUrl);

      // Verify that API key was added to URL parameters
      expect(mockAxios.get).toHaveBeenCalledWith(
